...
| 3 | TD-14R-P1 开箱即用的本机 Hermes 闭环 | 安装包内置 Hermes/Worker，项目读写/命令/电脑控制经真实审批和回执执行 | **是**（API/桌面/Hermes/真机） | 🔵 进行中（Codex，2026-07-25；已完成 Hermes-only 入口、真实本机 README 读取、ACP 回执/最终消息回传、路径边界收紧和会话刷新验证；M1-M4 的干净环境安装包、写入/命令审批、双平台发布仍未完成） |
| 4 | TD-15-T7 百人员工真实运行时验收 | 真 Hermes 同事请求安全抢占/恢复、macOS/Windows、多设备与 1-30 槽压测 | **是**（真实 Hermes/桌面/设备） | 🔵 进行中（Codex，2026-07-26；T1-T6、PostgreSQL 冷启动、真 Hermes WorkRequest 决策和 macOS Electron 本机只读 Run 已验，待抢占恢复、窄窗口、Windows 和 1-30 真槽证据） |
| 5 | TD-09-T3 剩余(渠道出站回复 + 微信/widget 适配器) | 渠道入站已通，出站未接 | 否（微信/widget 需真账号） | ⚪ |
| 6 | TD-08-T3 **剩余 UI 收尾**(空闲思考开关设置项，可选) | 前端 + IdleThinkService 均已完成✅ | 否（前端）| ⚪ |
...
| 任务 | commit | 备注 |
|---|---|---|
| **TD-10 受控业务工具门与真实邮件闭环**：独立 `/mcp/business-tools` + per-Run token；AgentPulse Fernet 加密凭证库；持久 `business_actions`/长期策略/审批轮询/租约重试；Resend 纯文本邮件 provider 与稳定幂等键；其余五类工具未配置时诚实失败；桌面端凭证、邮件渠道、审批恢复、动作审计和策略撤销。**真实 UI 验证**：录入/撤销假 key、刷新恢复审批、拒绝后 `attempt=0` 且 provider ID 为空；desktop/mobile 无溢出。338 passed / 9 skipped，desktop lint/build、三条架构 grep 全过；真实 Resend key 未提供，守护式真发信 E2E 跳过且本轮未发信 | `199f6a2` | [ADR 0011](../decisions/0011-controlled-business-actions.md) / [TD-10](TD-10-business-tool-gate.md)；下一步 TD-09 → TD-08 |
